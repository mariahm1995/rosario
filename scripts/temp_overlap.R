...
temp_overlap <- function(data, method, nsim) {
  require(gtools)
  ...
  for (i in 1:nrow(obs_overlap)) {
      ...
      if (method == "pianka") {
        obs_overlap[which(obs_overlap[,1] == ind1 & obs_overlap[,2] == ind2), 3] <- pianka_index(data[which(rownames(data) == ind1),], data[which(rownames(data) == ind2),])
      }
